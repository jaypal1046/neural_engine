...
const __dirname$1 = path.dirname(node_url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("bundles/main-BSmBJFD8.js", document.baseURI).href));
const isBundlesDir = path.basename(__dirname$1) === "bundles";
process.env.APP_ROOT = path.join(__dirname$1, isBundlesDir ? "../.." : "..");
...
function createWindow() {
  ...
  const preloadExists = node_fs.existsSync(preloadPath);
  electron.dialog.showMessageBoxSync({
    type: "info",
    title: "Preload Debug",
    message: `preloadPath: ${preloadPath}
...
electron.app.whenReady().then(() => {
  electron.ipcMain.handle("dialog:openFile", async () => {
    try {
      const parentWindow = electron.BrowserWindow.getFocusedWindow() || win;
      const options = { properties: ["openFile"] };
      const { canceled, filePaths } = parentWindow ? await electron.dialog.showOpenDialog(parentWindow, options) : await electron.dialog.showOpenDialog(options);
  ...
  electron.ipcMain.handle("dialog:saveFile", async () => {
    try {
      const parentWindow = electron.BrowserWindow.getFocusedWindow() || win;
      const options = {};
      const { canceled, filePath } = parentWindow ? await electron.dialog.showSaveDialog(parentWindow, options) : await electron.dialog.showSaveDialog(options);
  ...
  electron.ipcMain.handle("workspace:getRoot", () => {
    return activeWorkspaceRoot || null;
  });
