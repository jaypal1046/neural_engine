...
const BUNDLE_DIR = path.dirname(node_url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("bundles/main-GoECGQyn.js", document.baseURI).href));
const DIST_ELECTRON_DIR = path.basename(BUNDLE_DIR) === "bundles" ? path.dirname(BUNDLE_DIR) : BUNDLE_DIR;
const APP_ROOT = path.join(DIST_ELECTRON_DIR, "..");
process.env.APP_ROOT = APP_ROOT;
const VITE_DEV_SERVER_URL = process.env["VITE_DEV_SERVER_URL"];
const MAIN_DIST = DIST_ELECTRON_DIR;
...
const SERVER_HEALTH_URL = "http://127.0.0.1:8001/api/health";
...
async function isServerHealthy(timeoutMs = 1500) {
...
async function startServer() {
  ...
  serverStartupPromise = (async () => {
    ...
    if (await isServerHealthy()) {
      safeMainLog("log", "[Neural Studio] Reusing existing healthy Python server on 127.0.0.1:8001");
    ...
    serverProcess.on("exit", async (code) => {
      ...
      serverProcess = null;
      serverStartedByApp = false;
      if (await isServerHealthy(2e3)) {
        safeMainLog("log", "[Neural Studio] Existing Python server is healthy after child exit; continuing with shared local API.");
      }
    });
...
function stopServer() {
  if (serverProcess && serverStartedByApp) {
...
electron.app.whenReady().then(() => {
  electron.ipcMain.handle("dialog:openFile", async () => {
    try {
      const activeWindow = electron.BrowserWindow.getFocusedWindow() || win || void 0;
      const { canceled, filePaths } = await electron.dialog.showOpenDialog(activeWindow, {
        title: "Open File",
        defaultPath: normalizeWorkspaceRoot(activeWorkspaceRoot),
        properties: ["openFile"]
      });
      if (!canceled && filePaths[0]) {
        ...
        recordWorkspaceEvent("file_accessed", {
          ...
          source: "dialog_open_file"
        }, path.dirname(selectedFile));
        return selectedFile;
      }
    } catch (error) {
      safeMainLog("error", "[Neural Studio] Open file dialog failed", error);
    }
    return null;
  });
  ...
  electron.ipcMain.handle("dialog:openDirectory", async () => {
    try {
      ...
      if (!canceled && filePaths[0]) {
        ...
        activeWorkspaceRoot = normalizeWorkspaceRoot(selectedDir);
        recordWorkspaceEvent("workspace_selected", {
          source: "dialog_open_directory"
        }, selectedDir);
        return selectedDir;
      }
    } catch (error) {
      safeMainLog("error", "[Neural Studio] Open directory dialog failed", error);
    }
    return null;
  });
