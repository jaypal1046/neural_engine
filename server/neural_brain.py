...
def store_knowledge(topic: str, content: str, source: str = "user",
    ...
    # Auto-train C++ transformer on new knowledge
    try:
        engine = os.path.join(BASE_DIR, "bin", "neural_engine.exe")
        if os.path.exists(engine):
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8') as f:
                ...
                tmp = f.name
            try:
                subprocess.run([engine, "train", tmp], capture_output=True, timeout=30)
            finally:
                try: os.unlink(tmp)
                except: pass
    except Exception:
        pass  # Don't break learning if engine fails
...
def generate_response(query: str) -> dict:
    ...
    1. TF-IDF retrieval (find relevant documents)
    2. Transformer semantic matching (re-rank by meaning)
    3. Attention (focus on relevant sentences)
    4. Language model (generate coherent continuation)
    
    This combines retrieval + generation like a real LLM.
    """
    ensure_brain()
    
    # Step 1: Retrieve relevant knowledge with TF-IDF
    ...
    # Step 2: Get full text of top results
    context_parts = []
    sources = []
    ...
    if not context_parts:
        return {
            "response": "I found related topics but couldn't load the knowledge.",
            "sources": [{"topic": r["topic"], "score": r["score"]} for r in results[:3]],
            "confidence": 0.1,
        }
    
    # Step 3: Use transformer attention to find most relevant sentences
    all_sentences = []
    ...
    if not all_sentences:
        ...
        # Score sentences with both keyword matching and transformer similarity
        query_tokens = set(tokenize(query))
        scored_sentences = []
        
        # Check if C++ engine is available for semantic similarity
        engine = os.path.join(BASE_DIR, "bin", "neural_engine.exe")
        ...
        for sent in all_sentences:
            ...
            keyword_score = len(query_tokens & sent_tokens)
            
            # C++ transformer semantic score
            semantic_score = 0.0
            if has_engine and keyword_score > 0:
                try:
                    result = subprocess.run([engine, "similarity", query, sent],
                                           capture_output=True, text=True, timeout=5)
                    if result.returncode == 0:
                        import json as _json
                        data = _json.loads(result.stdout)
                        semantic_score = data.get("similarity", 0) * 3.0
                except Exception:
                    pass
    ...
    if has_engine:
        confidence = min(confidence * 1.2, 1.0)  # Boost confidence when transformer is active
